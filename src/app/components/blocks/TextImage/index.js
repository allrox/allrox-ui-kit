...
export default function TextImage({ children, image, width, height, alt, order, props, desc, maxwidth }) {
    ...
    return (
        <div className={`grid grid-cols-1 lg:grid-cols-2 justify-center gap-10 py-8 ${props}`}>
            ...
            <div className={`flex flex-col items-center justify-center ${order}`}>
                <Image src={image} width={width} height={height} alt={alt} loading="lazy" className={`flex rounded-lg w-full ${maxwidth}`} />
                {
                    desc && (
